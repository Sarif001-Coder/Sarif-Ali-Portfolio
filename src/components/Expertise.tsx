...
const Expertise = () => {
  ...
  const skills = [{
    ...
    text: "Functional Deployment"
  }, {
    icon: <GitBranch className="h-5 w-5" />,
    text: "GitHub Version Control"
  }, {
    icon: <Server className="h-5 w-5" />,
    text: "Linux Server Management"
  }, {
    icon: <Cloud className="h-5 w-5" />,
    text: "Cloud Infrastructure"
  }];
  const languages = ["Hindi (Limited Working Proficiency)", "English (Limited Working Proficiency)", "Bengali (Native Fluency)", "Gujarati (Native Fluency)"];
  return <section id="expertise" ref={ref} className={`py-16 px-6 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="space-y-3">
                {skills.map((skill, index) => <div key={index} className="flex items-center gap-3 text-foreground">
            ...
            <div>
              ...
              <ul className="space-y-2">
                {languages.map((lang, index) => <li key={index} className="flex items-center gap-3">
